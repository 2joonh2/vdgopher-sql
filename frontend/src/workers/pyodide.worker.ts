...
async function initPyodide() {
  try {
    ...
    console.log('Installing sqlglot...');
    await pyodide.loadPackage(['sqlglot']);
    console.log('sqlglot installed');
    
    // Load Python worker script
    console.log('Loading Python worker script...');
    const pythonCode = `
import sqlglot
...
`;
    ...
    console.log('Python worker script loaded');
...
self.onmessage = async (event) => {
  ...
  if (type === 'parse' && pyodide) {
    try {
      const { sql, dialect } = data;
      
      // Store values in Python globals to avoid escaping issues
      await pyodide.runPythonAsync(`
_sql = """${sql.replace(/"""/g, '\\"\\"\\"')}"""
_dialect = ${dialect ? `"${dialect}"` : 'None'}
`);
